...
def make_csv_if_not_exist(save_path: str, header: List = []):
    """
...
def write_to_csv_if_exists(save_path: str, data: List = []):
    """
...
def log_new_run_dir(save_dir: str = "exps/", K_fold_idx: int = 1):
    """
    ...
    - K_fold_idx: index of the current K-fold cross-validation run.
    ...
    # Append K-fold index
    dir_with_time_and_K_fold = dir_with_time + f"/Fold_{K_fold_idx}"
    _mkdirs_if_not_exist(dir_with_time_and_K_fold)

    return dir_with_time_and_K_fold

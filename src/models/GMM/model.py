...
class GMM:
    """
    Gaussian Mixture Model for Clustering Multi-Dimensional Time Series Data.
    """
    def __init__(self, input_shape, output_dim, model_name: str, K: int, random_state: int, 
                 *args, **kwargs):
        """
    ...
    def get_model_objects(self):
        ...
        # Unpack and convert
        means_ts = np.reshape(self.model.means_, (self.K, *self.input_shape[1:]))
        covariances_ts = np.reshape(self.model.covariances_, (self.K, *self.input_shape[1:]))

        return means_ts, covariances_ts, self.cluster_probs

...
def torch_gaussian_kl_div(mu_1, logvar_1, mu_2, logvar_2) -> torch.Tensor:
    """
    Computes KL divergence of two gaussian distributions with parameters mu_1, logvar_1 and mu_2, logvar_2

    Inputs: mu_1, logvar_1, mu_2, logvar_2 array-like of shape (batch_size, input_size)

    Outputs: KL divergence value averaged across batch.
    """

    # Compute individual terms
    term1 = torch.sum(logvar_2 - logvar_1, dim=-1, keepdim=False)
    term2 = torch.sum(torch.exp(logvar_1) / (torch.exp(logvar_2) + EPS), dim=-1, keepdim=False)
    term3 = torch.sum(((mu_2 - mu_1) ** 2) / (torch.exp(logvar_2) + EPS), dim=-1, keepdim=False)
    
    # Combine all terms
    kl_div = 0.5 * (term1 + term2 + term3)                # Shape (batch_size)
